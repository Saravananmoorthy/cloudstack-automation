...
class remoteSSHClient(object):
    def __init__(self, host, port, user, passwd):
        ...
        try:
            ...
            self.logger.info("connected to host %s"%self.host)
        except paramiko.SSHException, sshex:
            self.logger.info("failed to connect to host %s"%self.host)
            self.logger.debug(repr(sshex))
        
    def execute(self, command):
        self.logger.info("executing command %s on host %s"%(command,self.host)) 
