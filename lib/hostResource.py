...
import socket
import os
from hostImagerProfiles  import resourceManager, hostImager
import time
...
class hostManager:
    ...
    def _isPortOpen(self,hostQueue, port=22):
       ...
       while True:
           ...
           try:
              ...
              self.logger.debug("unable to connect to %s port %s due to %s"%(host,port,err))
           except socket.error, e:
              self.logger.info("encountered %s retrying in 5 sec"%e)
              err = e.errno
              delay(5)
           finally:
              if err == 0:
    ...
    def prepareManagementServer(self,url,branch,githubPrno,commit):
       """
       ...
       if self._isPortListening(host=self.mgmtHostInfo['ip'], port=22, timeout=300) \
          ...
          with contextlib.closing(remoteSSHClient(mgmt_ip, 22, "root", mgmt_pass)) as ssh:
            ...
            ssh.execute("python /root/buildAndDeploySimulator.py --package %s --noSimulator %s  -u %s %s  --fqdn %s %s -s True -o True > /var/log/cloudstack.log"%(package, self.mgmtHostInfo['noSimulator'], url, ("--githubPrno %s"%githubPrno if (githubPrno is not None) else "-b %s"%branch),  self.mgmtHostInfo['hostname']+"."+self.DOMAIN, ("" if commit==None else "-c %s"%commit)))
            buildlog="".join(ssh.stdout)
            if(len(ssh.errorlog) >0):
               raise Exception("failed to build cloudstack errorLog: %s"%ssh.errorlog)
    ...
    def create(self,repo_url, branch,githubPrno,commit,configName,profile):
       ...
       try:
          ...
          mgmtSsh.execute("puppet agent -t > /var/log/puppet.log ")
          ...
          if prepare_mgmt:
             compute_hosts=self.hostImager.imageHosts(self.mgmtHostInfo)
             self.prepareManagementServer(repo_url,branch,githubPrno,commit)
